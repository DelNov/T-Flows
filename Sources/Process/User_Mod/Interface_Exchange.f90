...
!==============================================================================!
  ...
  do d1 = 1, n_dom
    do d2 = 1, n_dom
      call Interface_Mod_To_Buffer(inter,                &
                                   flow(d1) % pnt_grid,  &
                                   flow(d2) % pnt_grid,  &
                                   flow(d1) % t % n,     &
                                   flow(d2) % t % n,     &
  ...
  do d1 = 1, n_dom
    do d2 = 1, n_dom
      ...
      ! (c1 and c2 here mean cell inside and on the boundary of domain 2)
      do n1 = 1, inter(d1, d2) % n1_sub
        ...
        wd1 = flow(d1) % pnt_grid % wall_dist(ic1)  ! wall distance in dom 1 1
...
! WRITE(100 + d1, *) t1, t2, k1, k2, wd1, wd2
      ...
      do n2 = 1, inter(d1, d2) % n2_sub
        ...
        t2  = flow(d2) % t % n(ic2)                 ! temperature in dom 1
        k2  = flow(d2) % conductivity(ic2)          ! conductivity in dom 1
        wd2 = flow(d2) % pnt_grid % wall_dist(ic2)  ! wall distance in dom 1 1
        t1  = inter(d1, d2) % phi_1(n, T)           ! temperature in dom 2
        k1  = inter(d1, d2) % phi_1(n, K)           ! conductivity in dom 2
        wd1 = inter(d1, d2) % phi_1(n, WD)          ! wall distance in dom 2
! WRITE(100 + d2, *) t1, t2, k1, k2, wd1, wd2
