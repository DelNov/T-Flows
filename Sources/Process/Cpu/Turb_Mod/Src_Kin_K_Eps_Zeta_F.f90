    ...
    if(Grid % region % type(reg) .eq. WALL .or.  &
      ...
      do s = Faces_In_Region(reg)
        ...
        if(Flow % buoyancy .eq. THERMALLY_DRIVEN) then
          ...
          ! production terms below.  This must NOT be written into
          ...
          ! overridden below with a log-law based estimate.  Sign follows
          ! the same convention as elsewhere (positive: heat flowing
          ! into the fluid).
          q_wall = t % q(c2)
