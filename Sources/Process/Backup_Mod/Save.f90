...
!==============================================================================!
  ...
  if(turbulence_model .eq. K_EPS) then
    ...
    call Backup_Mod_Write_Cell_Bnd(fh,d,vc, 'p_kin',  tur % p_kin (-nb_s:nc_s))
    call Backup_Mod_Write_Cell_Bnd(fh,d,vc, 'y_plus', tur % y_plus(-nb_s:nc_s))
    call Backup_Mod_Write_Cell_Bnd(fh,d,vc, 'vis_t',  tur % vis_t (-nb_s:nc_s))
    call Backup_Mod_Write_Cell_Bnd(fh,d,vc, 'vis_w',  tur % vis_w (-nb_s:nc_s))

    ! Turbulence quantities connected with heat transfer
    if(heat_transfer) then
      call Backup_Mod_Write_Variable(fh,d,vc, 't2',    tur % t2)
      call Backup_Mod_Write_Cell_Bnd(fh,d,vc, 'p_t2',  tur % p_t2 (-nb_s:nc_s))
      call Backup_Mod_Write_Cell_Bnd(fh,d,vc, 'con_w', tur % con_w(-nb_s:nc_s))
    end if
  ...
  if(turbulence_statistics .and.  &
    ...
    call Backup_Mod_Write_Cell_Bnd(fh,d,vc, 'u_mean', tur % u_mean(-nb_s:nc_s))
    call Backup_Mod_Write_Cell_Bnd(fh,d,vc, 'v_mean', tur % v_mean(-nb_s:nc_s))
    call Backup_Mod_Write_Cell_Bnd(fh,d,vc, 'w_mean', tur % w_mean(-nb_s:nc_s))
    call Backup_Mod_Write_Cell_Bnd(fh,d,vc, 'p_mean', tur % p_mean(-nb_s:nc_s))
    if(heat_transfer) then
    ...
    call Backup_Mod_Write_Cell_Bnd(fh,d,vc, 'ww_res', tur % ww_res(-nb_s:nc_s))
    call Backup_Mod_Write_Cell_Bnd(fh,d,vc, 'uv_res', tur % uv_res(-nb_s:nc_s))
    call Backup_Mod_Write_Cell_Bnd(fh,d,vc, 'uw_res', tur % uw_res(-nb_s:nc_s))
    call Backup_Mod_Write_Cell_Bnd(fh,d,vc, 'vw_res', tur % vw_res(-nb_s:nc_s))

    if(heat_transfer) then
      call Backup_Mod_Write_Cell_Bnd(fh,d,vc, 't2_res',tur % t2_res(-nb_s:nc_s))
      call Backup_Mod_Write_Cell_Bnd(fh,d,vc, 'ut_res',tur % ut_res(-nb_s:nc_s))
      call Backup_Mod_Write_Cell_Bnd(fh,d,vc, 'vt_res',tur % vt_res(-nb_s:nc_s))
      call Backup_Mod_Write_Cell_Bnd(fh,d,vc, 'wt_res',tur % wt_res(-nb_s:nc_s))
    end if
    ...
    if(fld % n_scalars > 0) then
      do sc = 1, fld % n_scalars
        ...
        name_mean = phi % name
        name_mean(5:9) = '_mean'
        call Backup_Mod_Write_Cell_Bnd(fh,d,vc, name_mean,  &
