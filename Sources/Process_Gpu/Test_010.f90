...
!------------------------------------------------------------------------------!
  ...
  integer                  :: nc, c, ldt
  logical                  :: exit_now
  character(11)            :: root_control = 'control.010'
!==============================================================================!

  ! Start the parallel run and the profiler
  call Global % Start_Parallel
  call Profiler % Start('Test_010')

  O_Print '(a)', ' #====================================================='
  O_Print '(a)', ' # TEST 10: Rayleigh-Benard convection case.'
  O_Print '(a)', ' #====================================================='
  ...
  if(Flow(1) % heat_transfer) then
    ...
    call Gpu % Vector_Real_Copy_To_Device(Flow(1) % t % x)
    call Gpu % Vector_Real_Copy_To_Device(Flow(1) % t % y)
    call Gpu % Vector_Real_Copy_To_Device(Flow(1) % t % z)
  end if
