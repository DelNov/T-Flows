!==============================================================================!
  subroutine Statistics(Cpu_Timer, indent)
!------------------------------------------------------------------------------!
  implicit none
!---------------------------------[Arguments]----------------------------------!
...
!==============================================================================!
  ...
  if(n_proc > 1) then
    do i_fun=1, Cpu_Timer % n_functions
      call Comm_Mod_Global_Sum_Real(Cpu_Timer % funct_time(i_fun))
      Cpu_Timer % funct_time(i_fun) = Cpu_Timer % funct_time(i_fun) / n_proc
    end do
  end if
  ...
  do
    swap = .false.
    do i_fun=1, Cpu_Timer % n_functions-1
      if(Cpu_Timer % funct_time(i_fun+1) > Cpu_Timer % funct_time(i_fun)) then
        t_temp = Cpu_Timer % funct_time(i_fun)
        n_temp = Cpu_Timer % funct_name(i_fun)
        Cpu_Timer % funct_time(i_fun)   = Cpu_Timer % funct_time(i_fun+1)
        Cpu_Timer % funct_name(i_fun)   = Cpu_Timer % funct_name(i_fun+1)
        Cpu_Timer % funct_time(i_fun+1) = t_temp
        Cpu_Timer % funct_name(i_fun+1) = n_temp
        swap = .true.
      end if
    end do
    if(.not. swap) goto 1
...
1 continue
  ...
  total_time = 0.0
  do i_fun=1, Cpu_Timer % n_functions
    total_time = total_time + Cpu_Timer % funct_time(i_fun)
  end do
  ...
  if(this_proc < 2) then
    ...
    do i_fun=1, Cpu_Timer % n_functions
      ...
      line( 5+indent: 5+indent+len_trim(Cpu_Timer % funct_name(i_fun)))  &
                            = Cpu_Timer % funct_name(i_fun)(1:123)
      line(45+indent:45+indent) = '|'
      percent_time = Cpu_Timer % funct_time(i_fun) / total_time * 100.0
