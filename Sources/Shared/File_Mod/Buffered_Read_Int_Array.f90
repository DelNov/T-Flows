!==============================================================================!
  subroutine Buffered_Read_Int_Array(File, file_unit, array)
!------------------------------------------------------------------------------!
!>  Reads integers from a file into an array using buffering. Buffering is
...
!       (BUFFER_SIZE).
!     - It then reads rem number of integers from the file into
!       File % i_buffer.
...
!---------------------------------[Arguments]----------------------------------!
  ...
  integer, intent(out) :: array(:)   !! array to be read from the file
!-----------------------------------[Locals]-----------------------------------!
...
!==============================================================================!
  ...
  do
    ...
    read(file_unit) File % i_buffer(1:rem)

    ! Store the File % buffered data in your data structure
    do i = 1, rem
      j = lo - 1 + tot + i
      array(j) = File % i_buffer(i)
    end do
