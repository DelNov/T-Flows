...
NC =  5;    // core between spirals
NZ = 21;
NI =  8;
NT  = 13;    // unit tangential resolution
...
Transfinite Curve {ms+0, ms+2, ms+5, ms+7} = NT*5+1 Using Progression 1;
Transfinite Curve {ms+1, ms+3, ms+4, ms+6} = NT*4+1 Using Progression 1;
...
Transfinite Curve {ms+0, ms+2, ms+4, ms+6} = NT*4+1 Using Progression 1;
Transfinite Curve {ms+1, ms+3, ms+5, ms+7} = NT*3+1 Using Progression 1;
...
Transfinite Curve {ms+0, ms+2, ms+4, ms+6} = NT*3+1 Using Progression 1;
Transfinite Curve {ms+1, ms+3, ms+5, ms+7} = NT*2+1 Using Progression 1;
...
Transfinite Curve {ms+0, ms+2, ms+4, ms+6} = NT*2+1 Using Progression 1;
Transfinite Curve {ms+1, ms+3, ms+5, ms+7} = NT  +1 Using Progression 1;
...
// These are in the core of the cold spirals
ms = m;
...
Transfinite Curve{m-2:m-1} = NT+1 Using Progression 1;
...
mc = m;
Transfinite Curve{ms:mc} = NC Using Progression 1;
...
mc = m;
Transfinite Curve{ms:mc} = 4 Using Progression 1;
...
Transfinite Curve {P_BND  :P_BND+1} = NT*5+1 Using Progression 1;
Transfinite Curve {P_BND+2:P_BND+3} = NT*5+1 Using Progression 1;
...
If(EXTRUDE == COLD)
  ...
  Physical Surface("walls") = {15193, 15051, 15073, 15095, 15117, 15139, 15161,
                               ...
                               15841, 15863, 15885, 12000, 12001, 12002, 12003,
                               12004, 12005, 12006, 12007, 12008, 12011, 12010,
                               12009, 12014, 12013, 12012, 12017, 12016, 12015,
                               12020, 12019, 12018, 12023, 12022, 12021,
                               2004, 2003, 2002, 2001, 2006, 2005, 2007};
  ...
  Physical Surface("pipes") = {15960, 15982, 16004, 15894, 15916, 15938,
                               16146, 16168, 16058, 16102, 16124, 16080};
EndIf
...
If(EXTRUDE == HOT)
  ...
  Physical Surface("walls") = {16105, 15533, 16127, 15555, 15929, 16017, 16039,
                               ...
                               15951, 15599, 15026, 15000, 15021, 15020, 15019,
                               15018, 15022, 15004, 15002, 15001, 15027, 15005,
                               15023, 15017, 15006, 15024, 15016, 15015, 15025,
                               15007, 15014, 15012, 15013, 15011, 15008, 15009,
                               15010, 15003,
                               1, 2, 4005, 4004, 4006, 4003, 4002, 4001, 4007};
  ...
  Physical Surface("pipes") = {16300, 16410, 16388, 16366, 16344, 16322,
                               16246, 16136, 16224, 16180, 16202, 16158};
EndIf
