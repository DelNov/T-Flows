  ...
  do i = 1, n_prob-1
    do c = 1, grid % n_cells - grid % comm % n_buff_cells 
      if(grid % zc(c) > (z_p(i)) .and.  &
        ...
        uu_p(i)    = uu_p(i) + uu % mean(c) &
                   - u % mean(c) * u % mean(c) 
        vv_p(i)    = vv_p(i) + vv % mean(c) &
                   - v % mean(c) * v % mean(c) 
        ww_p(i)    = ww_p(i) + ww % mean(c) &
                   - w % mean(c) * w % mean(c) 
        uw_p(i)    = uw_p(i) + uw % mean(c) &
                   - u % mean(c) * w % mean(c) 

        if(heat_transfer) then
          t_p(i)    = t_p(i) + t % mean(c)
          tt_p(i)   = tt_p(i) + tt % n(c) &
                      - t % mean(c) * t % mean(c)
          ut_p(i)   = ut_p(i) + ut % mean(c) &
                      - u % mean(c) * t % mean(c)
          vt_p(i)   = vt_p(i) + vt % mean(c) &
                      - v % mean(c) * t % mean(c)
          wt_p(i)   = wt_p(i) + wt % mean(c) &
                      - w % mean(c) * t % mean(c)
        end if
    ...
    do i = 1, n_prob
      if(n_count(i) .ne. 0) then
        write(3,'(12e15.7)') wall_p(i),                       &
                             ...
                             uw_p(i), 0.5*(uu_p(i)+vv_p(i)+ww_p(i)), &
                             t_p(i), tt_p(i), ut_p(i), vt_p(i), wt_p(i)
      end if
    end do
  else
    do i = 1, n_prob
